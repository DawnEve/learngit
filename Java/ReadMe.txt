...
第一章：编程基础 3-11 
    - 变量、运算符、流程控制; 类名和文件名一致
    - 变量范围；进制转换；装箱与拆箱; 
    - 综合练习(Integer/Demo2)：字符串是空格隔开的数字，输出排序后的字符串。
    - md5加密
    - 命令行参数
    - 可变参数
    
    - 字符串 本质是char的数组
    	* 获取字符串的编码格式 https://blog.51cto.com/u_5513510/6093795
...
第二章：数组 11 -31
	...
	- 枚举
第三章：OOP 面向对象程序开发 31 -74
	- 构造函数 //todo
	    * 构造器：执行顺序
    - 复写、重写 override、重载 的区别
	...
	- 三大特性
		...
		* 多态：同一个方法，传入不同的类做不同的处理。
			- 多态就是为了模糊父类和子类。Father s1=new Son();
			- 多态的实现方法: 重写、接口、抽象类与抽象方法
			- 多态编译时看父类是否有这个方法，运行时使用子类的具体方法。
    - 抽象类、抽象方法
    - 接口和抽象类： 
	...
	- finalize 方法: 在对象被垃圾收集器析构(回收)之前调用
    - 对象的相等
    - 静态static
    - 反射: 
    	* 代理: 静态代理、动态代理
    
    - 泛型： List<T> 泛型的本质就是为了安全
	    * 四种权限修饰符的不同点有哪些？
	    * 重写和重载的区别有哪些？
...
第八章：IO 流 199 -280
	...
	# 字符流
	B1 读取文件，写入文件
	B2 拷贝文件夹，考虑子文件夹；文件加密与解密；排序文件中-分割的数字
	# 缓冲流
	C1 读写数据 缓冲流：字符，字节
	C2 测试：4种字节流的拷贝速度比较
	C3 示例：对序号开头的文本按照序号排序
	# 转换流
	D1 转换文件编码
	D2 利用字节流读取文件，一次读取一行，不能乱码
	# 序列化流
	E1 java对象保存到文件中
	# 打印流
	F1 win 下的标准输出怎么写？
...
第二十章 算法 第四版
	section1: 牛顿迭代法求平方根, 二分法查找递归实现, 三角函数(弧度)和对数(自然对数)
